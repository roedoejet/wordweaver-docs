...
function pluginContentDocs(context, opts) {
    ...
    return {
        ...
        async contentLoaded({ content, actions }) {
            ...
            const genRoutes = async (metadataItems) => {
                ...
                await metadataItems.forEach(async (metadataItem, i) => {
                    var _a, _b;
                    const isDocsHomePage = metadataItem.id.substr(metadataItem.id.indexOf('/') + 1) ===
                        options.homePageId;
                    if (isDocsHomePage) {
                        const homeDocsRoutePath = routeBasePath === '' ? '/' : routeBasePath;
                        const versionDocsPathPrefix = (_a = ((metadataItem === null || metadataItem === void 0 ? void 0 : metadataItem.version) === versioning.latestVersion
                            ? ''
                            : metadataItem.version)) !== null && _a !== void 0 ? _a : '';
                        // To show the sidebar, get the sidebar key of available sibling item.
                        metadataItem.sidebar = ((_b = metadataItems[i - 1]) !== null && _b !== void 0 ? _b : metadataItems[i + 1]).sidebar;
                        const docsBaseMetadata = createDocsBaseMetadata(metadataItem.version);
                        docsBaseMetadata.isHomePage = true;
                        docsBaseMetadata.homePagePath = utils_1.normalizeUrl([
                            baseUrl,
                            homeDocsRoutePath,
                            versionDocsPathPrefix,
                            options.homePageId,
                        ]);
                        const docsBaseMetadataPath = await createData(`${utils_1.docuHash(metadataItem.source)}-base.json`, JSON.stringify(docsBaseMetadata, null, 2));
                        // Add a route for docs home page.
                        addRoute({
                            path: utils_1.normalizeUrl([
                                baseUrl,
                                homeDocsRoutePath,
                                versionDocsPathPrefix,
                            ]),
        ...
        async routesLoaded(routes) {
            ...
            const homeDocsRoutes = routes.filter((routeConfig) => routeConfig.path === normalizedHomeDocsRoutePath);
